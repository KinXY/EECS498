...
class Conv(object):
    ...
    def backward(dout, cache):
        ...
        # padded x, for convenience
        pad_x = torch.nn.functional.pad(x, (pad, pad, pad, pad))
        # grad
        dw = torch.zeros_like(w)
        dx = torch.zeros_like(x)
        db = torch.zeros_like(b)
        dx_pad = torch.zeros_like(pad_x)
        for i in range(N):
            for j in range(F):
                for k in range(H_out):
                    for l in range(W_out):
...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        self.params["W2"] = (
            torch.randn(
                num_filters * input_dims[1] * input_dims[2] // 4,
                hidden_dim,
                dtype=dtype,
                device=device,
            )
    ...
    def loss(self, X, y=None):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        result_1, cache_1 = Conv_ReLU_Pool.forward(X, W1, b1, conv_param, pool_param)
        result_2, cache_2 = Linear_ReLU.forward(result_1, W2, b2)
        scores, cache_3 = Linear.forward(result_2, W3, b3)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        grad_result_2, grads["W3"], grads["b3"] = Linear.backward(grad_scores, cache_3)
        grad_result_1, grads["W2"], grads["b2"] = Linear_ReLU.backward(
            grad_result_2, cache_2
        )
        _, grads["W1"], grads["b1"] = Conv_ReLU_Pool.backward(grad_result_1, cache_1)
        ###################################################################
        #                             END OF YOUR CODE                    #
        ###################################################################
...
class DeepConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            if i in self.max_pools:
                if self.batchnorm:
                    ...
                    Xt, cache = Conv_BatchNorm_ReLU_Pool.forward(
                        Xt,
                        w,
                        b,
                        gamma,
                    ...
                    Xt, cache = Conv_ReLU_Pool.forward(
                        Xt,
                        w,
                        b,
                        conv_param,
                ...
                if self.batchnorm:
                    ...
                    beta = self.params[f"beta{i + 1}"]
                    bn_param = self.bn_params[i]
                    Xt, cache = Conv_BatchNorm_ReLU.forward(
                        Xt,
                        w,
                        b,
                        gamma,
        ...
        ###################################################################
        # Replace "pass" statement with your code
        loss, dXt = softmax_loss(scores, y)
        loss += self.reg * sum(
            torch.sum(self.params[f"W{i+1}"] ** 2) for i in range(self.num_layers)
        )
        # linear layer
        dXt, grads[f"W{self.num_layers}"], grads[f"b{self.num_layers}"] = (
            Linear.backward(dXt, caches[-1])
        )
        grads[f"W{self.num_layers}"] += (
            2 * self.reg * self.params[f"W{self.num_layers}"]
        )
        # conv layers
            ...
            if i in self.max_pools:
                if self.batchnorm:
                    (
                        ...
                        grads[f"gamma{i + 1}"],
                        grads[f"beta{i + 1}"],
                    ) = Conv_BatchNorm_ReLU_Pool.backward(dXt, caches[i])
                else:
                    dXt, grads[f"W{i + 1}"], grads[f"b{i + 1}"] = (
                        Conv_ReLU_Pool.backward(dXt, caches[i])
                    )
            else:
                if self.batchnorm:
                    (
                        ...
                        grads[f"gamma{i + 1}"],
                        grads[f"beta{i + 1}"],
                    ) = Conv_BatchNorm_ReLU.backward(dXt, caches[i])
                else:
                    dXt, grads[f"W{i + 1}"], grads[f"b{i + 1}"] = Conv_ReLU.backward(
                        dXt, caches[i]
                    )
...
def kaiming_initializer(
    ...
    if K is None:
        ...
        # Replace "pass" statement with your code
        weight = (
            torch.randn(Din, Dout, dtype=dtype, device=device) * (gain / Din) ** 0.5
        )
        ###################################################################
        #                            END OF YOUR CODE                     #
        ###################################################################
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == "train":
            ...
            x_zero_mean = x - sample_mean
            sample_var = (x_zero_mean**2).mean(dim=0)
            out = x_zero_mean / torch.sqrt(sample_var + eps)
            out = gamma * out + beta
            # update running mean and variance
            running_mean = momentum * running_mean + (1 - momentum) * sample_mean
            running_var = momentum * running_var + (1 - momentum) * sample_var
            # cache
            cache = (mode, x, gamma, beta, x_zero_mean, sample_mean, sample_var, eps)
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == "test":
            ...
            out = gamma * out + beta
            cache = (mode, x, gamma, beta, running_mean, running_var, eps)
            ################################################################
            #                      END OF YOUR CODE                        #
            ################################################################
        else:
    ...
    def backward(dout, cache):
        ...
        mode = cache[0]
        if mode == "train":
            mode, x, gamma, beta, x_zero_mean, sample_mean, sample_var, eps = cache
            N = x.shape[0]
            sqrt_var = torch.sqrt(sample_var + eps)
            x_hat = x_zero_mean / sqrt_var
            # start backprop
            d_x_hat = dout * gamma
            dgamma = torch.sum(dout * x_hat, dim=0)
            dbeta = dout.sum(dim=0)
            # d_var
            d_inverse_sqrt_var = torch.sum(d_x_hat * x_zero_mean, dim=0)
            d_sqrt_var = -(sqrt_var**-2) * d_inverse_sqrt_var
            d_var = 0.5 * sqrt_var**-1 * d_sqrt_var
            # d_mean
            d_no_mean = 2 / N * x_zero_mean * d_var
            d_no_mean += d_x_hat / sqrt_var
            d_mean = -d_no_mean.sum(dim=0)
            # dx
            dx = d_no_mean + d_mean / N
        elif mode == "test":
            mode, x, gamma, beta, running_mean, running_var, eps = cache
            sqrt_var = torch.sqrt(running_var + eps)
            x_hat = (x - running_mean) / sqrt_var
            dgamma = torch.sum(dout * x_hat, dim=0)
            dbeta = dout.sum(dim=0)
            dx = gamma * dout / sqrt_var
        else:
    ...
    def backward_alt(dout, cache):
        ...
        if mode == "train":
            ...
            d_no_mean = gamma * (
                dout / sqrt_var
                - 1 / N * x_zero_mean * (dout * x_zero_mean).sum(dim=0) / sqrt_var**3
            )
            dx = d_no_mean - d_no_mean.sum(dim=0) / N
        elif mode == "test":
            mode, x, gamma, beta, running_mean, running_var, eps = cache
            sqrt_var = torch.sqrt(running_var + eps)
            x_hat = (x - running_mean) / sqrt_var

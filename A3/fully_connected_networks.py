...
class TwoLayerNet(object):
    ...
    def __init__(
        ...
        self.params["W2"] = (
            torch.randn(hidden_dim, num_classes, device=device, dtype=dtype)
            * weight_scale
        )
    ...
    def loss(self, X, y=None):
        ...
        scores, cache2 = Linear_ReLU.forward(
        ...
        # grad
        dhidden, grads["W2"], grads["b2"] = Linear_ReLU.backward(dscores, cache2)
        _, grads["W1"], grads["b1"] = Linear_ReLU.backward(dhidden, cache1)
...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(self.num_layers):
            ...
            # initialization
            self.params[f"W{i+1}"] = (
                torch.randn(in_dim, out_dim, device=device, dtype=dtype) * weight_scale
            )
            self.params[f"b{i+1}"] = torch.zeros(out_dim, device=device, dtype=dtype)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            if i == 0:
                hidden_i, cache_i = Linear_ReLU.forward(
                    X, self.params[f"W{i+1}"], self.params[f"b{i+1}"]
                )
            elif i == self.num_layers - 1:
                hidden_i, cache_i = Linear.forward(
                    hidden[i - 1], self.params[f"W{i+1}"], self.params[f"b{i+1}"]
                )
            else:
                hidden_i, cache_i = Linear_ReLU.forward(
                    hidden[i - 1], self.params[f"W{i+1}"], self.params[f"b{i+1}"]
            ...
            if self.use_dropout and i != self.num_layers - 1:
                hidden_i, cache_dropout = Dropout.forward(hidden_i, self.dropout_param)
                cache_i = (cache_i, cache_dropout)
        ...
        for i in range(self.num_layers, 0, -1):
            # backpropagate through the dropout, if there is one
            if self.use_dropout and i != self.num_layers:
                dhidden = Dropout.backward(dhidden, cache[i - 1][1])
                cache[i - 1] = cache[i - 1][0]
            loss += self.reg * torch.sum(self.params[f"W{i}"] ** 2)
            if i == self.num_layers:
                dhidden, grads[f"W{i}"], grads[f"b{i}"] = Linear.backward(
                    dhidden, cache[i - 1]
                )
            else:
                dhidden, grads[f"W{i}"], grads[f"b{i}"] = Linear_ReLU.backward(
                    dhidden, cache[i - 1]
                )
...
def rmsprop(w, dw, config=None):
    ...
    next_w -= (
        config["learning_rate"] * dw / (torch.sqrt(config["cache"]) + config["epsilon"])
    )
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == "train":
            ...
            ##############################################################
            # Replace "pass" statement with your code
            mask = (torch.rand(x.shape, device=x.device) > p).to(x.dtype) / (1 - p)
            out = x * mask
            ##############################################################
            #                   END OF YOUR CODE                         #
            ##############################################################

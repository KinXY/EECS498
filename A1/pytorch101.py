...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    start_tens = (start // 10) * 10
    if start_tens < start:
        start_tens += 10
    stop_tens = (stop // 10) * 10
    if start_tens > stop_tens:
        x = torch.tensor([], dtype=torch.float64)
    else:
        x = torch.arange(start_tens, stop_tens + 1, 10, dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def slice_assignment_practice(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    x[:4, :6] = torch.tensor(
        [[0, 1, 2, 2, 2, 2], [0, 1, 2, 2, 2, 2], [3, 4, 3, 4, 5, 5], [3, 4, 3, 4, 5, 5]]
    )
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    idx_x = range(0, y.shape[0])
    y[idx_x, row_min_idx] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    means = x.sum(dim=0) / x.shape[0]
    stds = ((x - means) ** 2).sum(dim=0) / (x.shape[0] - 1)
    stds = stds.sqrt()
    y = (x - means) / stds

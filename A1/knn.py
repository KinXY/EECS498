...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test):
            dists[i, j] = torch.sum(
                (x_train[i].reshape(-1) - x_test[j].reshape(-1)) ** 2
            )
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        dists[i] = (
            x_test.reshape(num_test, -1).sub(x_train[i].reshape(-1)).pow(2).sum(dim=1)
        )
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    dists = dists.add(x_train.reshape(num_train, -1).pow(2).sum(dim=1).unsqueeze(1))
    dists = dists.add(x_test.reshape(num_test, -1).pow(2).sum(dim=1))
    dists = dists.sub(
        2 * torch.mm(x_train.reshape(num_train, -1), x_test.reshape(num_test, -1).t())
    )
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        top_k = torch.topk(dists[:, i], k, largest=False)
...
class KnnClassifier:
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        y_test_pred = predict_labels(
            compute_distances_no_loops(self.x_train, x_test), self.y_train, k
        )
        ######################################################################
        #                         END OF YOUR CODE                           #
        ######################################################################
    ...
    for k in k_choices:
        for i in range(num_folds):
            ...
            k_to_accuracies[k].append(
                classifier.check_accuracy(val_data, val_labels, k=k, quiet=True)
            )
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k in k_to_accuracies:
        if best_k == 0 or sum(k_to_accuracies[k]) / len(k_to_accuracies[k]) > sum(
            k_to_accuracies[best_k]
        ) / len(k_to_accuracies[best_k]):
            best_k = k
        elif (
            sum(k_to_accuracies[k]) / len(k_to_accuracies[k])
            == sum(k_to_accuracies[best_k]) / len(k_to_accuracies[best_k])
            and k < best_k
        ):
            best_k = k
    ##########################################################################
